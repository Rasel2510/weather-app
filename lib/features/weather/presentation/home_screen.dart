...
class _HomeState extends State<Home> {
  ...
  Future<Weather> _fetchWeather({String? cityName}) async {
    ...
    bool connected = await NetworkHelper.isConnected();
    if (!connected) {
      // Show popup immediately in the main thread
      WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<Weather>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            // Check if error contains "No internet" to show specific message
            final error = snapshot.error.toString();
            bool isNetworkError = error.contains("No internet");
              ...
              child: Column(
                ...
                children: [
                  Icon(
                    ...
                    color: isNetworkError ? Colors.red : Colors.cyanAccent,
                    size: 60,
                  ),
                  SizedBox(height: 20),
                  Text(
                    isNetworkError ? "No internet connection" : "Error loading weather",
                    style: TextStyle(
                      color: Colors.cyanAccent,
                      fontSize: 18,
                    ),
